package com.kuloud.android.permission;
...
public class RationaleDialogFragmentCompat extends AppCompatDialogFragment {
    ...
    private Permission.PermissionCallbacks mPermissionCallbacks;
    private Permission.RationaleCallbacks mRationaleCallbacks;
    ...
    public void onAttach(Context context) {
        ...
        if (getParentFragment() != null) {
            if (getParentFragment() instanceof Permission.PermissionCallbacks) {
                mPermissionCallbacks = (Permission.PermissionCallbacks) getParentFragment();
            }
            if (getParentFragment() instanceof Permission.RationaleCallbacks){
                mRationaleCallbacks = (Permission.RationaleCallbacks) getParentFragment();
            }
        }

        if (context instanceof Permission.PermissionCallbacks) {
            mPermissionCallbacks = (Permission.PermissionCallbacks) context;
        }

        if (context instanceof Permission.RationaleCallbacks) {
            mRationaleCallbacks = (Permission.RationaleCallbacks) context;
        }
    }
